...
class ExampleFloodIt {
  ...
  void initializeFlood() {
    ...
    makeGame = new FloodItWorld(10,5);
  }
  ...
  void testWorld(Tester t) {
    initializeFlood();
    t.checkExpect(makeGame.boardSize, 10);
    t.checkExpect(makeGame.colors, 5);
    makeGame.initializeGame(10, 8);
  }

 
  // Tests for changing the color of neighbors of a cell
  void testChangeNeighbors() {
 
  }

 
  // Tests for adjusting the scale of cells depending
  // on the board size.
  void testSetScale(Tester t) {
  }

 
  // Tests for returning the cell at the position
  // of the mouse click received by updateOnClick.
  void testCellPos(Tester t) {
  }

 
  // Tests for checking changing the origin
  // cell after a mouse click on the board.
  void testChangeOrigin(Tester t) {
    
    FloodItWorld testWorld = new FloodItWorld(3, 3);
    Cell testCell = new Cell(0, 0, 3, true);
    testCell.color = Color.blue;
    
    testWorld.changeOrigin(testCell);
  }

 
  // Tests for the position information received
  // by a mouse click.
  void testOnMouseClicked(Tester t) {
  }
 
  // Tests for floodCells to ensure that it
  // Updates neighbor cells of the same color to flooded.
  void testFloodCells(Tester t) {
  }

 
  // Tests to ensure the game ends when all cells on the
  // board are flooded.
  void testGameOver(Tester t) {
  }

 
  // Tests for checking every cell on the board to see if 
  // they are all flooded.
  void testCheckAllCellsForFlooded(Tester t) {
  }
